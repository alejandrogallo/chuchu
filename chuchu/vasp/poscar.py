...
class Poscar(object):
    ...
    def getAtomSymbol(self, atom_number):
        self.logger.debug("Getting %s atom symbol"%atom_number)
        if atom_number == 0:
    ...
    def getCoordinates(self, atom_number):
        ...
        return coords
    def load(self, obj):
        self.logger.debug("Loading poscar from %s"%obj)
        if type(obj) is file:
            f = obj
        ...
        for j,line in enumerate(f):
            ...
            if line_number == 1:
                ...
                self.logger.debug("constant = %s"%line)
                self.constant = float(line)
            elif 5>=line_number >= 3:
                self.logger.debug("vector (%s) = %s"%(line_number-2, line))
                self.basis.append([float(i) for i in re.sub(r"\s+"," ", line).split()])
            elif line_number == 6:
                self.atoms_header = re.sub(r"\s+"," ", line).split()
            elif line_number == 7:
                self.atoms_number_header = [int(x) for x in re.sub(r"\s+"," ", line).split()]
            elif line_number == 8:
                self.mode=line
    ...
    def dumpVasp(self):
        ...
        contents = template.substitute(
                ...
                coordinates = "\n".join(
                    [" ".join([str(c) for c in vector]) for vector in self.atoms]
                    )
                )
        return contents
    def dumpAsyAtoms(
        ...
        for symbol in self.getAtomSymbols():
            draw_atoms.append(
"""\
ALL_ATOMS.drawAtom(\"%s\", draw_label = false, radius_scale =\
radius_scale);\
"""%(symbol))
        for j,symbol in enumerate(self.getAtomSymbols()):
            for i,symbol2 in enumerate(self.getAtomSymbols()):
                if j<i:
                    ...
                    draw_bonds.append("""\
ALL_ATOMS.drawBond(\"%s\", \"%s\", bond_radius = bond_radius,\
max_dist = max_bond_dist);"""%( symbol, symbol2))
